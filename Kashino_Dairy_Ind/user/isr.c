...
void elec_brake()
{
    if(start_line_flag)
    {
        if(right_encoder_dir<=80 || stoped)
        {
            left_duty=0;
            right_duty=0;
            stoped=1;
...
IFX_INTERRUPT(cc60_pit_ch0_isr, CCU6_0_CH0_INT_VECTAB_NUM, CCU6_0_CH0_ISR_PRIORITY)
{
    ...
    if(start_flag)
    {
        right_duty = right_motor_PI(&right_PID,right_encoder_dir,right_encoder_exp_dir);
        left_duty  = left_motor_PI(&left_PID,left_encoder_dir,left_encoder_exp_dir);

        if(right_duty>=Max_Speed) right_duty=Max_Speed;
        else if(right_duty<=-Max_Speed) right_duty=-Max_Speed;
        if(left_duty>=Max_Speed) left_duty=Max_Speed;
        else if(left_duty<=-Max_Speed) left_duty=-Max_Speed;

        elec_brake();
        motor_driver(right_duty,left_duty);             //ATOM0_CH5_P02_5        ATOM0_CH7_P02_7
    }
    else motor_driver(0,0);
...
IFX_INTERRUPT(cc60_pit_ch1_isr, CCU6_0_CH1_INT_VECTAB_NUM, CCU6_0_CH1_ISR_PRIORITY)
{
    ...
    right_encoder_dir = 6 * encoder_get_count(RIGHT_ENCODER_DIR);                          // 获取编码器计数
    left_encoder_dir = -6 * encoder_get_count(LEFT_ENCODER_DIR);                           // 获取编码器计数

    encoder_clear_count(RIGHT_ENCODER_DIR);                                                 // 清空编码器计数
    encoder_clear_count(LEFT_ENCODER_DIR);                                                  // 清空编码器计数
...
IFX_INTERRUPT(cc61_pit_ch0_isr, CCU6_1_CH0_INT_VECTAB_NUM, CCU6_1_CH0_ISR_PRIORITY)
{
    ...
    printf("%d, %d, %d, %d, %d, %d, %d\n",left_encoder_dir,left_encoder_exp_dir,right_encoder_dir,right_encoder_exp_dir,Increase_left,Increase_right,center_line);
}
...
//    gyro_angle_y += gyro_y * 0.005;                               //判断是否为坡道，强制中线为91
    gyro_angle_z += gyro_z * 0.005;                               //右转为负，判断圆环角度，障碍物转向角度

//    angle_y = gyro_angle_y * 0.6 + acc_angle_y * 0.4;

    if(gyro_angle_z >= 360)  gyro_angle_z = 0;
    if(gyro_angle_z <= -360) gyro_angle_z = 0;
